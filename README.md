...
- **Behind a corporate proxy, every CLI fetch hangs while the browser works fine.** macOS hands browsers a PAC file through system settings; curl, git, and anything written in Go never read it. They attempt a direct connection, the network drops the packets instead of refusing them, and you get a hang with no error to read. `scutil --proxy` tells you a PAC is in play and names it. The proxy values themselves belong in `~/.zshrc.local`, which the zshrc sources last and this repo deliberately never manages: a gateway hostname belongs to whoever owns the network, not to a public repo.
- **A 403 from a proxy is better news than a timeout.** `CONNECT tunnel failed, response 403` means the proxy answered and refused you, so the route is fine and what's left to sort out is which gateway you picked or whether the device is enrolled. A timeout means nothing is listening at all. Read which one you got before hunting for a firewall rule that isn't there.
- **`no_proxy` doesn't understand globs.** `*.example.com` matches nothing, and curl sends those hosts through the proxy anyway without complaining. A bare `example.com` already covers its subdomains, which is what you meant. Same for address ranges: CIDR works, `10.0.*.*` doesn't.
- **Go's HTTP client is the first thing to break through an inspecting proxy.** chezmoi's builtin git and the 1Password CLI are both Go binaries, and both die with `unexpected EOF` mid-request where curl sails through. Real git is more tolerant, so drop `--use-builtin-git=on` once the Command Line Tools are in; `git config --global http.version HTTP/1.1` handles what's left. For a Go tool with no such knob, `GODEBUG=http2client=0` turns off its HTTP/2 client.
