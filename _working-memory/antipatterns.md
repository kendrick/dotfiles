...
**Why we backed out:** Only for the interleaving cases. The separation is load-bearing everywhere else: under a pty a combined capture cannot tell an animation drawn correctly to `/dev/tty` from one wrongly drawn to stdout, so #37's channel clauses need the streams apart. #38 needs them together, so `pty-run.py --stdout-to-tty` gives the separation up on purpose and leaves fd 1 on the pty slave. It refuses to run with `--setsid`, which opens no pty and so offers no slave for fd 1 to land on.
**Don't suggest:** Adding an interleaving assertion to a venue that keeps the two streams apart, and don't collapse the channel venues to match the interleaving one. The two modes measure different properties and both are needed. Any new interleaving case asserts `glyph_rows` and `marker_rows` are non-zero alongside `violations`, because a capture that recorded no frames reports zero violations while proving nothing.

## 2026-08-31 — Don't expect the ten-script venue to exercise install-claude-plugins
**Tried:** Driving `run_onchange_after_install-claude-plugins.sh.tmpl` through `ui_ten_venue` for #38's interleaving case, the way the other nine scripts go through it.
**What broke:** The rendered script was a two-line early exit. The `claude plugin` calls the case existed to watch were never written into it, so the case passed against a script that does nothing.
**Why we backed out:** That template picks its branch at render time by stat-ing `~/.config/chezmoi/key.txt`. With no age key in the venue's fake `$HOME`, the branch carrying the install calls is never emitted, and no amount of stubbing at run time can reach code that isn't there. `ui_claude_plugins_venue` generates a throwaway age keypair and encrypts its own fixtures to it, so the render takes the real branch and no test reaches for the user's key.
**Don't suggest:** Adding install-claude-plugins to `ui_ten_venue`'s script list, or stubbing `claude` harder to make a case pass there. Don't point a test at the real `~/.config/chezmoi/key.txt` either. When a template branches on a file's existence at render time, that file has to exist in the fixture, so read the rendered output before believing a green case.

## 2026-08-31 — Don't clear the collided row by buffering, and don't put the ticker back
**Tried:** Two repairs for #38 that never shipped. First, buffer a tool's output and replay it once the step settles. Second, once `step_run` has erased the live line, restart the ticker before handing control back, so the animation survives the tool call.
**What broke:** Buffering breaks the 2026-08-06 error-handling decision. A failing tool's stderr has to stay on screen beside the item that failed, and a slow tool's scrollback is the only liveness signal there is while it holds the run, which `npx skills add` and `brew` both do for minutes at a time. Restarting the ticker rebuilds the same collision one line further down: the caller's own failure `echo` runs immediately after `step_run` returns, and a live line back up sits in front of it.
**Why we backed out:** We took the cost instead. In a loop that ticks and then immediately shells out, the live line is drawn and erased inside a millisecond, so those four steps are carried by their settled line and the tool's own output rather than by the animation. That is the intended behavior of `step_run`, not a regression.
**Don't suggest:** A buffer, a `tee`, or a `2>&1` capture replayed after `step_ok` to keep the live line clean. Don't add a ticker restart to the end of `step_run`, and don't file the missing animation on those four steps as a bug.
